...
router.get("/:id", async (req,res)=>{
  ...
  if(pool){
    if (pool) {
      console.log("hola")
      const result = await sql.query( `
      SELECT c.IdCaso, c.FechaOcurrencia, c.FechaSolicitud, c.Diagnostico, c.CantDias, c.CantHorasDias, c.Idcaso, pa.IdPaciente, pa.Dni, pa.Nombre as NombrePaciente, pa.Direccion, pa.Localidad, pa.Telefono, pa.FechaNacimiento, pr.Nombre as NombrePrestador, c.IdSituacion
